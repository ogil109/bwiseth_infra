...
# Create Subnet
def create_subnet(name: str, vpc_id: str, cidr_block: str, availability_zone: str, map_public_ip: bool) -> aws.ec2.Subnet:
    """
    Create a subnet.
    ...
    """
    try:
        return aws.ec2.Subnet(f"{name}-subnet",
                             vpc_id=vpc_id,
                             cidr_block=cidr_block,
...
def create_internet_gateway(name: str, vpc_id: str) -> aws.ec2.InternetGateway:
    ...
    """
    try:
        return aws.ec2.InternetGateway(f"{name}-igw",
                                       vpc_id=vpc_id,
...
# Create NAT Gateway
def create_nat_gateway(name: str, subnet_id: str, allocation_id: str, depends_on) -> aws.ec2.NatGateway:
    """
    ...
    Args:
        ...
        depends_on: Dependencies for the resource.

    Returns:
        aws.ec2.NatGateway: The created NAT Gateway resource.
    """
    try:
        return aws.ec2.NatGateway(f"{name}-nat-gateway",
                                  subnet_id=subnet_id,
...
def create_vpc_environment(name: str, cidr_block: str) -> None:
    ...
    try:
        ...
        public_subnet1 = create_subnet(f"{name}-public-subnet1", vpc.id, get_subnet_cidr(cidr_block, 1), zones[0], True)
        public_subnet2 = create_subnet(f"{name}-public-subnet2", vpc.id, get_subnet_cidr(cidr_block, 2), zones[1], True)

        private_subnet1 = create_subnet(f"{name}-private-subnet1", vpc.id, get_subnet_cidr(cidr_block, 3), zones[0], False)
        private_subnet2 = create_subnet(f"{name}-private-subnet2", vpc.id, get_subnet_cidr(cidr_block, 4), zones[1], False)
        ...
        nat_gateway1 = create_nat_gateway(f"{name}-nat-gateway1", public_subnet1.id, eip1.id, [internet_gateway])
        nat_gateway2 = create_nat_gateway(f"{name}-nat-gateway2", public_subnet2.id, eip2.id, [internet_gateway])
